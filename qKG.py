...
obj_func_name      = "MyFunc"
job_id             = 0
objective_func     = obj.Obj(np.array(conf["bounds"]), conf["num_init"], conf["var_name"])
dim                = objective_func._dim
num_fidelity       = objective_func._num_fidelity
num_initial_points = int(objective_func._num_init_pts)
num_to_sample      = conf["batch_size"]
num_iteration      = conf["max_iter"]
...
for n in xrange(num_iteration):
    ...
    print "KG, VOI {0}, best so far {1}".format(voi, objective_func.evaluate_true(0, report_point)[0])
